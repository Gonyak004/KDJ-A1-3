...
- 소스코드 저장소 내에 .env 파일을 일체 작성하지 않으며, 환경 변수를 파일 형태로 저장하지 않고 클라우드 호스팅 인프라에 직접 주입하는 방식을 채택했습니다. 이를 통해 로컬 환경변수 파일 또는 Git 커밋 및 노출 위험을 근본적으로 차단했습니다.
...
- 프론트엔드는 API 키를 전혀 소유하지 않으며, 백엔드 API 엔드포인트(/api/app)로 필요한 파라미터만 전달합니다.
